#!/usr/bin/env python3
"""Standalone GaugePack v1 LayerNorm RSQRT NN-LUT trainer.

Frozen arithmetic contract
--------------------------

    D   = 128 * sum(z_i**2) - sum(z_i)**2          # U30
    x   = D27 = RNE_even(D / 16)                    # positive A27 input
    rho = 1 / sqrt(16*x + epsilon_D)                # real teacher
    R   = U8(RNE_even(rho / s_rho))                 # 0..255

The learned reference model is a continuous 15-hinge ReLU spline, hence a
16-segment piecewise-linear function:

    f(x) = b + m*x + sum_i delta_m[i] * relu(x-knot[i])

This file deliberately stops at the floating PWL artifact.  It does NOT claim
to have compiled the page into GaugePack's production M18/C48/P48 format.  A
separate coefficient compiler must quantize the exported knots/slopes/
intercepts and then rerun the bit-true regression.
...
Both calibration and disjoint-search files use:
    ...
    x_int32 : D27 values, shape [N], integer, strictly positive
    count   : optional occurrence count/weight, shape [N], positive

Repeated x_int32 entries are coalesced automatically.  The calibration file is
the only source used to choose s_rho and fit parameters.  The search file is
opened only for held-out metrics.  SST-2 validation must not be supplied here.
...
-------
    ...
    python gaugepack_rsqrt_nnlut_train.py \
      ...
      --out-dir ./rsqrt_layer0_attention \
      --device cuda
...
# GaugePack v1 frozen constants.
SEED = 20260728
SEGMENT_COUNT = 16
HINGE_COUNT = SEGMENT_COUNT - 1
HIDDEN_SIZE = 128
D_PRE_SHIFT = 4
D27_MAX = (1 << 26) - 1  # Positive values carried by signed DSP A27.
RSQRT_QMIN = 0
RSQRT_QMAX = 255
EPSILON_D_CODE = 0
INT32_MIN = -(1 << 31)
...
def seed_everything(seed: int) -> None:
    """Seed Python, NumPy, and PyTorch without enabling TF32."""
...
def round_shift_rne(values: np.ndarray | int, shift: int) -> np.ndarray:
    """Signed integer RNE-even division by 2**shift."""
    ...
    quotient = np.floor_divide(x, divisor)
...
class Histogram:
    ...
    def validate(self, role: str) -> None:
        ...
        if np.any(x <= 0):
            raise ValueError(
                f"{role}: D27 must be strictly positive; D=0 rows bypass RSQRT"
            )
        if int(x.max()) > D27_MAX:
            raise ValueError(
                f"{role}: D27 max {int(x.max())} exceeds signed A27 positive "
                f"limit {D27_MAX}"
            )
...
def load_histogram_npz(path: Path, role: str) -> Histogram:
    ...
    with np.load(path, allow_pickle=False) as payload:
        ...
        if "count" in payload.files:
            weight = np.asarray(payload["count"], dtype=np.float64)
        else:
            weight = np.ones(np.asarray(x).size, dtype=np.float64)
    return coalesce_histogram(x, weight, f"{role}:{path}")


def rsqrt_teacher_real(x_d27: np.ndarray) -> np.ndarray:
    """Return 1/sqrt(16*D27) for the frozen epsilon_D=0 contract."""

    x = ensure_int32("D27 teacher input", x_d27)
    radicand = x.astype(np.float64) * float(1 << D_PRE_SHIFT)
    if EPSILON_D_CODE != 0:
        radicand += float(EPSILON_D_CODE)
    if np.any(radicand <= 0.0):
        raise ValueError("RSQRT teacher radicand must be positive")
    return 1.0 / np.sqrt(radicand)


def choose_rho_scale(calibration_x: np.ndarray, requested: Optional[float]) -> float:
    """Choose s_rho from calibration only; search data is never consulted."""

    if requested is not None:
        if not math.isfinite(requested) or requested <= 0.0:
            raise ValueError("--rho-scale must be finite and positive")
        return float(requested)
    maximum = float(rsqrt_teacher_real(calibration_x).max())
    return max(maximum / float(RSQRT_QMAX), np.finfo(np.float64).tiny)


def rsqrt_target_code(x_d27: np.ndarray, rho_scale: float) -> np.ndarray:
    if not math.isfinite(rho_scale) or rho_scale <= 0.0:
        raise ValueError("rho_scale must be finite and positive")
    code = np.rint(rsqrt_teacher_real(x_d27) / rho_scale).astype(np.int64)
    return np.clip(code, RSQRT_QMIN, RSQRT_QMAX)
...
def weighted_metrics(
    ...
    return {
        ...
        "target_saturation_high_rate": float(w[ref == RSQRT_QMAX].sum() / total),
        "predicted_saturation_high_rate": float(
            w[pred == RSQRT_QMAX].sum() / total
        ),
    }
...
class OrderedHingePWL(nn.Module):
    ...
    def __init__(
        ...
        if initial_knots_raw is None:
            knots_u = np.linspace(
                -1.0, 1.0, HINGE_COUNT + 2, dtype=np.float64
            )[1:-1]
        else:
        ...
        gaps /= gaps.sum()
        raw_gap = np.log(np.expm1(gaps))

        self.raw_gap = nn.Parameter(torch.tensor(raw_gap, dtype=torch.float64))
    ...
    def knots_raw(self) -> torch.Tensor:
        return self.x_center + self.x_scale * self.knots_u()
    ...
    def initialize_least_squares(
        ...
        """Initialize line coefficients while holding the initial knots fixed."""
        ...
        if x.size > max_points:
            ...
            probability = w / w.sum()
            chosen = rng.choice(
                x.size, size=max_points, replace=False, p=probability
            )
            x = x[chosen]
            y = y[chosen]
            w = w[chosen]
    ...
    def raw_lines_numpy(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Return raw-D27 knots, slopes, and intercepts for all 16 segments."""
        ...
        cumulative_delta = np.concatenate(
            ([0.0], np.cumsum(delta, dtype=np.float64))
        )
        slope_u = initial_slope + cumulative_delta
        cumulative_offset = np.concatenate(
            ([0.0], np.cumsum(delta * knots_u, dtype=np.float64))
        )
...
def initial_knots(x: np.ndarray) -> np.ndarray:
    """Match the current GaugePack flow: quantiles over observed D27 support."""

    raw = ensure_int32("knot initialization x", x)
    if int(raw.max()) - int(raw.min()) < HINGE_COUNT:
        raise ValueError("calibration D27 domain is too narrow for 16 segments")
    probabilities = np.linspace(
        0.0, 1.0, HINGE_COUNT + 2, dtype=np.float64
    )[1:-1]
    return np.quantile(raw.astype(np.float64), probabilities)


def ste_round_clamp(
    values: torch.Tensor, qmin: int, qmax: int
) -> torch.Tensor:
    """Hard RNE/clamp forward pass with identity gradient backward."""

    hard = torch.clamp(torch.round(values), qmin, qmax)
...
def predict_numpy(
    ...
    with torch.no_grad():
        for begin in range(0, source.size, batch_size):
            ...
            code = torch.clamp(
                torch.round(model(tensor)), RSQRT_QMIN, RSQRT_QMAX
            )
...
class TrainedRSQRTPage:
    ...
    def export(self, directory: Path) -> Dict[str, str]:
        ...
        payload = {
            "schema": "gaugepack.rsqrt.float_pwl_training.v1",
            "name": self.name,
            "task": "rsqrt",
            ...
            "qmin": RSQRT_QMIN,
            "qmax": RSQRT_QMAX,
            "rho_output_scale": self.rho_scale,
            "input_policy": "clamp_to_calibration_domain",
            "compiler_status": "NOT_COMPILED_M18_C48",
            "deployment_equations": {
                "D": "128*sum(z^2)-sum(z)^2",
                "D27": "RNE_even(D/16)",
                "teacher": "rho=1/sqrt(16*D27)",
                "output": "R=clamp_U8(RNE_even(rho/s_rho))",
                "downstream": "T=(128*z-S)*R; no intermediate shift",
            },
        ...
        torch.save(
            {
                "schema": "gaugepack.rsqrt.trainer_checkpoint.v1",
                "name": self.name,
                ...
                "frozen_contract": {
                    ...
                    "D_pre_shift": D_PRE_SHIFT,
                    "epsilon_D_code": EPSILON_D_CODE,
                    "output_code_range": [RSQRT_QMIN, RSQRT_QMAX],
                },
            },
            checkpoint_path,
        )
...
def evaluate_histogram(
    *,
    ...
    reference = rsqrt_target_code(histogram.x, rho_scale)
...
def train_rsqrt_page(
    *,
    ...
    target = rsqrt_target_code(x, rho_scale)
    x_min = int(x.min())
    x_max = int(x.max())
    ...
    if config.train_dtype == "float32" and x_max > (1 << 24):
        print(
            "[warning] D27 exceeds exact float32 integer resolution; "
    ...
    model.initialize_least_squares(
        x,
        target,
        weight,
        seed=config.seed,
    ...
    normalized_weight = weight / max(float(weight.mean()), 1.0e-12)
    w_tensor = torch.as_tensor(
        normalized_weight, dtype=dtype, device=device
    )
    ...
    for epoch in range(config.epochs):
        ...
        for begin in range(0, x.size, config.batch_size):
            ...
            hard_prediction = ste_round_clamp(
                raw_prediction, RSQRT_QMIN, RSQRT_QMAX
            )
            weight_sum = torch.sum(wb)
            hard_l1 = torch.sum(
                wb * torch.abs(hard_prediction - yb)
            ) / weight_sum
            float_l1 = torch.sum(
                wb * torch.abs(raw_prediction - yb)
            ) / weight_sum
    ...
    metadata = {
        ...
        "input_kind": "D27=RNE_even((128*Q-S^2)/16)",
        "input_scale_folded": float(1 << D_PRE_SHIFT),
        "epsilon_D_code": EPSILON_D_CODE,
        "D0_policy": "bypass page and emit rho=0",
        "rho_output_scale": rho_scale,
        "rho_output_quant": "U8 [0,255]",
        "rho_scale_selected_from": "calibration_only",
        "search_role": "held_out_metrics_only",
        "validation_used": False,
        "compiler_required_next": "production M18/C48/P48 compile and bit-true regression",
    }
    return TrainedRSQRTPage(
        name=name,
        model=model,
        x_min=x_min,
        x_max=x_max,
...
def run_training(args: argparse.Namespace) -> Dict[str, Any]:
    if args.calibration_npz is None or args.search_npz is None:
        raise ValueError(
            "--calibration-npz and --search-npz are both required"
        )
    ...
    rho_scale = choose_rho_scale(calibration.x, args.rho_scale)
    config = FitConfig(
        epochs=args.epochs,
    ...
    manifest = {
        "schema": "gaugepack.rsqrt.training_run.v1",
        "frozen_contract": {
            "hidden_size": HIDDEN_SIZE,
            "D_definition": "128*Q-S^2",
            "D_pre_shift": D_PRE_SHIFT,
            "D27_positive_max": D27_MAX,
            "epsilon_D_code": EPSILON_D_CODE,
            "teacher": "1/sqrt(16*D27)",
            "output_code": "U8 [0,255]",
        },
    ...
    print(f"rho output scale s_rho={rho_scale:.17g}")
...
def run_self_test() -> None:
    """Run math checks plus a short end-to-end synthetic training/export."""

    source = np.asarray([8, 24, 40, -8, -24, -40], dtype=np.int64)
    expected = np.asarray([0, 2, 2, 0, -2, -2], dtype=np.int64)
    observed = round_shift_rne(source, 4)
    if not np.array_equal(observed, expected):
        raise AssertionError(f"RNE-even self-test failed: {observed} != {expected}")

    cal_x = np.unique(
        np.rint(np.geomspace(1_000, 10_000_000, 512)).astype(np.int64)
    )
    search_x = np.unique(
        np.rint(np.geomspace(1_050, 9_500_000, 384)).astype(np.int64)
    )
    cal_w = 1.0 + (cal_x.max() - cal_x) / float(cal_x.max()) * 7.0
    search_w = 1.0 + (search_x.max() - search_x) / float(search_x.max()) * 5.0
    ...
    rho_scale = choose_rho_scale(calibration.x, None)
    page = train_rsqrt_page(
        name="selftest_rsqrt",
        calibration=calibration,
        search=search,
    ...
    with tempfile.TemporaryDirectory(prefix="gaugepack_rsqrt_selftest_") as tmp:
    ...
    print(
        "Self-test passed: RNE-even D>>4, positive D27 contract, "
        "15 ordered hinges, 16 float lines, U8 codes, and artifact reload."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Train one GaugePack LayerNorm D27->U8 RSQRT 16-segment "
            "NN-LUT reference page."
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--calibration-npz", type=Path)
    parser.add_argument("--search-npz", type=Path)
    parser.add_argument("--site-name", default="rsqrt_site")
    parser.add_argument(
        "--out-dir", type=Path, default=Path("gaugepack_rsqrt_nnlut_artifact")
    )
