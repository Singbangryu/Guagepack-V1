...
module vfu_dsp16 (
    ...
    input  wire [287:0] b_i,  // 16 x S18
    input  wire [767:0] c_i,  // 16 x S48
    output reg          valid_o,
    output reg  [3:0]   op_o,
    output wire [767:0] p_o   // 16 x S48
);
    // -------------------------------------------------------------------------
    ...
    generate
        for (lane = 0; lane < 16; lane = lane + 1) begin : GEN_LANE
            wire signed [26:0] a_lane = a_i[lane*27 +: 27];
            wire signed [17:0] b_lane = b_i[lane*18 +: 18];
            wire signed [47:0] c_lane = c_i[lane*48 +: 48];

            // DSP A port is 30 bits; the multiplier consumes signed A27.
            wire [29:0] dsp_a_w = {{3{a_lane[26]}}, a_lane};
            wire [17:0] dsp_b_w = b_lane;
            wire [47:0] dsp_c_w = c_lane;
